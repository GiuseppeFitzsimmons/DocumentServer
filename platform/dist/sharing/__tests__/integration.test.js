...
vi.mock('jsonwebtoken', () => ({
    default: {
        sign: vi.fn((payload) => `signed-token-${JSON.stringify(payload).slice(0, 20)}`),
        verify: vi.fn(),
    },
}));
import { pool } from '../../db/pool.js';
import { createShare, getShare, listSharedFiles, revokeShare, deleteSharesForFile, } from '../service.js';
import { buildEditorConfig } from '../../ds/editorConfig.js';
const mockQuery = pool.query;
// --- Helper to seed in-memory data ---
function seedUser(id, email, displayName) {
    store.users.set(id, { id, email, display_name: displayName });
}
function seedFile(id, userId, name, updatedAt = '2024-06-15T10:00:00Z') {
    store.files.set(id, {
        id,
        user_id: userId,
        name,
...
function addShareToStore(fileId, ownerId, inviteeId, permissions, createdAt = new Date().toISOString()) {
    shareIdCounter++;
...
function configureMockQuery() {
    mockQuery.mockImplementation((sql, params) => {
        ...
        if (query.startsWith('INSERT INTO file_shares')) {
            const [fileId, ownerId, inviteeId, edit, download, print, copy, comment, review, chat, fillForms] = params;
            shareIdCounter++;
            const id = `share-${shareIdCounter}`;
