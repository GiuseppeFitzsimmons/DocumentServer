...
import { createShare, updateSharePermissions, type SharePermissions } from '../service.js';
  ...
  it('updateSharePermissions rejects all-false permissions with status 400', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        uuidArb,
        allFalsePermissionsArb,
        async (shareId, ownerId, permissions) => {
          mockQuery.mockReset();
    ...
    fc.assert(
      fc.property(
        ...
        (file, user, sharePermissions) => {
          const result = buildEditorConfig({ file, user, sharePermissions }) as {
