...
const validPermissionsArb: fc.Arbitrary<SharePermissions> = fc
  ...
  .filter(
    (p) =>
      p.edit || p.download || p.print || p.copy || p.comment || p.review || p.chat || p.fillForms
  );

/** Generates a SharePermissions object with ALL flags false */
const allFalsePermissionsArb: fc.Arbitrary<SharePermissions> = fc.constant({
  edit: false,
  download: false,
  print: false,
  copy: false,
  ...
  it('creating a share and reading it back yields exact same permission flags', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        emailArb,
        validPermissionsArb,
        async (fileId, ownerId, inviteeId, inviteeEmail, permissions) => {
          mockQuery.mockReset();
  ...
  it('creating a share with a non-registered email fails with 404', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        emailArb,
        validPermissionsArb,
        async (fileId, ownerId, inviteeEmail, permissions) => {
          mockQuery.mockReset();
  ...
  it('non-owner attempting to create a share returns 403', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        emailArb,
        validPermissionsArb,
        async (fileId, ownerId, actualOwnerId, inviteeEmail, permissions) => {
  ...
  it('non-owner attempting to revoke a share returns 403', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        uuidArb,
        uuidArb,
        async (shareId, requesterId, actualOwnerId) => {
  ...
  it('non-owner attempting to list shares for a file returns 403', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        uuidArb,
        uuidArb,
        async (fileId, requesterId, actualOwnerId) => {
          fc.pre(requesterId !== actualOwnerId);
          mockQuery.mockReset();
  ...
  it('updating permissions and reading back yields exact new values', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        uuidArb,
        validPermissionsArb,
        async (shareId, ownerId, newPermissions) => {
          mockQuery.mockReset();
  ...
  it('after owner revokes a share, querying that share returns null', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        uuidArb,
        uuidArb,
        async (shareId, fileId, ownerId, inviteeId) => {
          mockQuery.mockReset();
...
describe('Property 14: File deletion cascades to shares', () => {
  it('after calling deleteSharesForFile, no shares remain for that file ID', async () => {
    await fc.assert(
      fc.asyncProperty(uuidArb, async (fileId) => {
        mockQuery.mockReset();
