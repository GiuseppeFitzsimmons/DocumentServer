...
pageRouter.post('/login', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id, email, display_name, password_hash, is_temp_password FROM users WHERE email = $1',
      [email]
    );
...
pageRouter.post('/set-password', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE users SET password_hash = $1, is_temp_password = false WHERE id = $2',
